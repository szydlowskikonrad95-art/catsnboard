/**
 * Front-end view script for `catsnboard/services`.
 *
 ...
 * reveals, training photo) because render.php emits the same classes/IDs.
...
( function () {
	...
	function themeDrivesAnimations() {
		...
		return (
			...
			document.querySelector( '.cnb-services .splitw .wi' ) !== null
		);
	}
	...
	function ensureVisible() {
		...
		document
			.querySelectorAll( '.cnb-services [data-rev], .cnb-services .svc' )
			.forEach( function ( el ) {
	...
	function run() {
		if ( ! document.querySelector( '.cnb-services' ) ) {
			return;
		}

/**
 * Front-end view script for `catsnboard/about`.
 *
 * IMPORTANT — no double animation.
 * The Cats'N'Board THEME already enqueues assets/js/main.js site-wide, and that
 * script fully animates this markup (split-words, .svc reveal, [data-rev]
 * reveals) because render.php emits the same classes/IDs. Running GSAP again
 * here would double-wrap words and double-init ScrollTrigger. So this view
 * script is a GUARD: it only acts as a fallback when the theme JS is NOT present
 * (e.g. the block used in a different theme).
 *
...
( function () {
	...
	function themeDrivesAnimations() {
		...
		return (
			...
			document.querySelector( '.cnb-about .splitw .wi' ) !== null
		);
	}

	function ensureVisible() {
		// Fallback only: the theme CSS keeps [data-rev] visible by default; the
		// only elements the theme JS hides before revealing are .svc. If the
		// theme JS never runs, make sure nothing is stuck invisible.
		document
			.querySelectorAll( '.cnb-about [data-rev], .cnb-about .svc' )
			.forEach( function ( el ) {
	...
	function run() {
		if ( ! document.querySelector( '.cnb-about' ) ) {
			return;
		}

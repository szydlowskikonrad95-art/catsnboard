/**
 * Front-end view script for `catsnboard/pricing`.
 *
 * IMPORTANT — no double animation.
 * The Cats'N'Board THEME already enqueues assets/js/main.js site-wide, and that
 * script fully animates this markup (split-words, [data-rev] reveals, .pcard
 * hover) because render.php emits the same classes/IDs. Running GSAP again here
 * would double-wrap words and double-init ScrollTrigger. So this view script is a
...
( function () {
	...
	function themeDrivesAnimations() {
		...
		return (
			...
			document.querySelector( '.cnb-pricing .splitw .wi' ) !== null
		);
	}

	function ensureVisible() {
		// Fallback only: the theme CSS keeps [data-rev] visible by default. If
		// the theme JS never runs, make sure nothing is stuck invisible.
		document
			.querySelectorAll( '.cnb-pricing [data-rev]' )
			.forEach( function ( el ) {
	...
	function run() {
		if ( ! document.querySelector( '.cnb-pricing' ) ) {
			return;
		}
